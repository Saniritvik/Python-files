...
def fillGrid():
    ...
    for i in range(1, 10):
        filled = True
        while filled:
            xturn = int(input("Player " + turn +
                        " where would you like to pick? (1-9)"))
...
def gamemode():
    question = input(
        "Would you like to play 2 players or fight against a pc? (pc/pvp)")
    if question == "pc":
        fillGridPc()
    else:
        fillGrid()
